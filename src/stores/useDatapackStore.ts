...
export const useDatapackStore = defineStore('datapacks', () => {
    ...
    const lithosphereReferenceJar = new OptionalUrlDatapack("prism_mod_jars/lithosphere-1.6.jar", metadata.datapackFormat, "Lithosphere reference jar")
    const stillLifeReferenceJar = new OptionalUrlDatapack("prism_mod_jars/still-life-0.1.1.jar", metadata.datapackFormat, "Still Life reference jar")
    const referenceJarsEnabled = ref(false)
    ...
    const datapacks = reactive<DatapackEntry[]>(ds)
    const visibleDatapacks = computed(() => [...datapacks])
    ...
    settingsStore.$subscribe(async () => {
        ...
        lithosphereReferenceJar.setPackVersion(metadata.datapackFormat)
    ...
    const datapack_list = new class implements DatapackList{
        async getDatapacks(): Promise<AnonymousDatapack[]> {
            return [
                ...datapacks.map(d => d.datapack),
                ...(referenceJarsEnabled.value ? [lithosphereReferenceJar, stillLifeReferenceJar] : []),
                prismTuningDatapack
            ]
        }
    }

    const baseline_datapack_list = new class implements DatapackList{
        async getDatapacks(): Promise<AnonymousDatapack[]> {
            return [
                ...datapacks.map(d => d.datapack),
                ...(referenceJarsEnabled.value ? [lithosphereReferenceJar, stillLifeReferenceJar] : [])
    ...
    const baseline_composite_datapack = Datapack.compose(baseline_datapack_list)
    ...
    function clearRemovableDatapacks() {
        referenceJarsEnabled.value = false
        datapacks.splice(1, datapacks.length - 1)
        notifyTuningChanged()
    }

    function notifyTuningChanged() {
        for (const listener of tuningListeners) listener()
    }

    function onTuningChanged(listener: () => void) {
    ...
    return { datapacks: visibleDatapacks, composite_datapack, baseline_composite_datapack, registered, reloadDatapack, notifyTuningChanged, onTuningChanged, addDatapack, addInstanceModDatapacks, clearRemovableDatapacks, removeDatapack, dimensions, world_presets, addModrinthDatapack }
})
