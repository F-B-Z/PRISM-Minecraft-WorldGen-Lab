...
export const useSettingsStore = defineStore('settings', () => {
    ...
    datapackStore.$subscribe(async () => {
        const worldPresets = await datapackStore.world_presets
        if (worldPresets.length > 0 && worldPresets.findIndex((id) => id.equals(world_preset.value)) === -1) {
            world_preset.value = worldPresets[0]
        }

        const dimensions = await datapackStore.dimensions
        if (dimensions.length > 0 && dimensions.findIndex((id) => id.equals(dimension.value)) === -1) {
